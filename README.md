...
分析师研究六模式工具包：访谈提纲 / 全覆盖观点汇总 / 研报 PPTX / 单篇深度转述 / 会议纪要 / 猹狸百科。
...
| **E · 会议纪要** | 有一段访谈逐字稿（语音转文字，英文或中文），需要整理成标准格式的 Word 人工纪要 | 中文逐字稿 Markdown + Word 人工纪要（楷体 12pt，标题、Key Takeaway、Q&A 三部分）+ AI 纪要对标分析（如提供 AI 纪要） |
| **F · 猹狸百科** | 刚入行的医药 intern 需要快速学习公司/赛道/术语/估值/财报基础 | HTML 学习报告（Inter 字体、浅米底、slate navy accent）+ 可选 Markdown |
...
# 模式 E：会议纪要
# 在 Claude Code 中输入：
# "帮我弄一份人工纪要，逐字稿在这里 [逐字稿路径]，参考纪要格式 [参考纪要路径]"
```
...
| 文件 | 用途 |
|------|------|
| `SOP.md` | **完整参考手册**：五模式全部细节、示例、故障排查 |
| `SKILL.md` | **操作摘要**：五模式核心流程、格式骨架、核对清单（Claude Code skill 入口） |
...
原始逐字稿（语音转文字，通常英文或中文）→ Word 人工纪要 + 中文逐字稿 Markdown + AI 对标分析：

1. **格式校准**：先读取用户提供的参考纪要 docx，提取字体（楷体 12pt）、Key Takeaway 结构、Q&A 格式（Q 加粗/A 不加粗）
2. **逐字稿拆解**：识别讲话人、判断语音识别归因错误（提问者的话常被归入讲话人段落），按对话时间顺序拆分 Q&A，去掉口语填充词。**Q&A 严格按时序，禁止按主题重排。**
3. **中文逐字稿 Markdown**：去填充词，保留关键语气和态度信号
4. **Word 人工纪要**：标题（日期+对象+会议纪要）、Key Takeaway（≤5 个主题组，每条 bullet 一个投资判断，≤60 字）、Q&A（第一人称，逐字稿原话去填充词，零箭头零破折号，追问链用 Re Q / Re A）
5. **AI 纪要对标**：如用户提供了 AI 纪要，逐题对比遗漏、过度确定化、定性信号丢失、张冠李戴

核心铁律：
- **按时序不重排**：Q&A 严格按逐字稿对话顺序，不按主题归类
- **零箭头（→）、零破折号（——）**
- **逐字不概括**：纪要 A = 逐字稿删填充词和口头禅，中间没有"概括"步骤
- **Key Takeaway 是判断不是数据附录**：数据细节放 Q&A
...
# 生成纪要
# 在 Claude Code 中输入：
# "帮我弄一份人工纪要，逐字稿：[路径]，参考纪要：[路径]，AI 纪要：[路径]（可选）"
```

## 模式 F 输出格式

公司/赛道/术语/估值问题 → HTML 学习报告（带教模式，面向刚入行医药 intern）：

1. **六个核心模块**：一句话定位、赛道归类、业务拆分、药物机制（药企必含）、竞争优势、财务与估值
2. **术语速查**：报告末尾的迷你词典，分医药术语和财务术语两栏
3. **文风**：允许"我们拆开看"式引导，每个 jargon 首次出现跟"小白提示"，不确定信息标注【未确认】
4. **设计**：Inter 字体、浅米底、slate navy accent、琥珀色 callout 框

```bash
# 在 Claude Code 中输入：
# "帮我了解XX公司""这个赛道怎么样""估值怎么看""什么是靶点/Ph2/适应症"
```
