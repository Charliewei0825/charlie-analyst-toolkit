...
分析师研究五模式工具包：访谈提纲 / 全覆盖观点汇总 / 研报 PPTX / 单篇深度转述 / 会议纪要。
...
原始逐字稿（语音转文字，通常英文）→ Word 人工纪要 + 中文逐字稿 Markdown + AI 对

标分析：

1. **格式校准**：先读取用户提供的参考纪要 docx，提取字体（楷体 12pt）、Key Takeaway 结构、Q&A 格式（Q 加粗/A 不加粗）
2. **逐字稿拆解**：识别讲话人、按对话时间顺序拆分 Q&A、去掉口语填充词
3. **中文逐字稿 Markdown**：英文原话转述为中文（保留关键术语英文、标注讲话人身份）
4. **Word 人工纪要**：标题（日期 + 对象 + 会议纪要）、Key Takeaway（按主题归并，一句话一事实，带具体数字）、Q&A（第一人称，零箭头零破折号）
5. **AI 纪要对标**：如用户提供了 AI 纪要，逐题对比遗漏、定性偏差、细节遗失

核心格式铁律：
- **零箭头（→）、零破折号（——）**：句子之间用句号和逗号连接
- **第一人称回答**：回答者说什么就是什么，不加第三人称夹注，不过度润色
- **Key Takeaway 极简**：主题标题不加修饰，每条要点一句话讲完一个事实
