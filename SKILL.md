...
- 联网搜索（Claude Code 需配 Tavily MCP key 于 `~/.claude.json` user scope；Codex 自带 web_search，无需额外配置）
...
⚠️ **Q&A 最终输出严格按逐字稿时间顺序排列。禁止按主题打散重排。** 对话的跳跃、回头、追问链本身就是信息——按主题归类会抹平这些信号。

⚠️ **语音转文字经常把提问者的话归入讲话人段落。** 必须从上下文判断每句话的实际讲话人。例如逐字稿标注"讲话人1"的一大段里可能夹杂了提问者的追问——不能照录成马教授的自述。
...
- **讲话人标签可能不准**：提问者的追问常被语音识别归入回答方段落，需从语义判断实际讲话人
...
**Step 1.5 — 非1v1会议与大量叙述场景的特殊处理（2026-07-06 建投医药余老师教训）**

当会议不是标准 1v1 访谈、某一方有大量单方面叙述（如主讲人翻 PPT 介绍公司/行业情况）时，Step 1 的"识别问答边界"的前提就不成立——大部分内容根本没有问答边界。此时必须切换处理模式。
...
| **话题标签** | 逐字稿里没人问，是主讲人在单方面铺陈 | "诺华相关情况""默沙东相关情况""诺华BTK相关" | 主讲人单方面叙述 |

第二种是轻量 section header，借用 Q 格式只是为了视觉一致性，**不是假装有人问了这个问题**。
...
**多发言人对话追踪：Re Q / Re A / 具名标签**：

当会议有 3+ 人参与、主讲人在铺陈过程中被多次打断/追问/补充时，使用以下标签：

| 标签 | 含义 | 何时用 |
|------|------|--------|
| `Re Q：` | 追问/插话 | 提问方在同一话题下继续追问 |
| `Re A：` | 回应 | 主讲人对追问的回答 |
| `王老师：`（具名） | 第三方专家发言 | 非主讲人、非提问方的第三方插入 |

示例（建投医药余老师纪要，IL-6 话题）：
```
Q：诺华IL-6心血管品种相关情况
A：IL-6很重要，但是目前只有二期数据……[余老师介绍，多段]
...
Re A：[余老师说他不太了解 BioAge]

Re Q：[提问方补充 BioAge 市值和催化剂逻辑]

王老师：[再次插入，讨论诺华心血管方向是否在收缩]
```

**关键规则**：
- Re Q / Re A 保留对话节奏，但不给每个追问起独立 Q 标题
- 具名标签让读者知道换了发言人
- 所有内容严格按逐字稿时间顺序，不按主题重排
- 同一个话题下的连续多轮讨论（如 Lp(a) 线性的 6 轮 Re 链）保留完整推理过程

**判断流程**（每次写纪要前过一遍）：
```
1. 这是 1v1 访谈还是多人会议？
   → 1v1：传统 Q&A 格式
   → 多人/有大量叙述：启用话题标签 + Re 机制
...
**Step 1.7 — 1v1 访谈开头独白的处理（2026-07-09 Josh Cantor 教训）**

即使为标准 1v1 访谈，语音转文字逐字稿经常出现开头一大段只有讲话人 1（回答方）在输出、没有对应的提问录音的情况。这是因为录音设备可能在提问方说话时才开始录，或者提问方的开场问题没有被清晰收音。

**处理规则**：这段独白不能跳过，也不是"话题标签"（因为确实是有人问了什么才引出的回答）。处理方式是：

```
1. 在逐字稿中标注"【以下为开头独白，提问未录到】"
2. 在纪要中构建一个重建 Q，格式：
   Q：（录音未录到提问，根据回答内容推测：[简要概括推测的提问内容]）
3. 然后正常跟 A 段落
```

**典型场景**（Josh Cantor 案例）：
```
Q：（录音未录到提问，根据回答内容推测：现在致幻剂抗抑郁赛道各家临床数据信息不完整，商业化前景看不清，Compass、Atai、GH Research 几家公司各有优劣，同时行业估值分化很大，这条赛道到底值不值得重仓？我们该怎么判断市场空间、怎么挑选个股、要不要押单一龙头？）

A：每次给药之后还有很多细节公司没有分享。数据可以有很多种切法……
```

**判断标准**：
- 逐字稿开头 ≥3 段 讲话人 1 的连续输出且夹杂不下于 2 个独立话题 → 是独白，需要重建 Q
- 如果仅仅是 1-2 句开场寒暄 → 直接省略，不重建 Q

**Step 1.8 — Re Q / Re A 适用范围扩展（2026-07-09 Josh Cantor 教训）**

Re Q / Re A 标签不仅适用于 3+ 人会议，在 1v1 访谈中**当一个追问链超过 2 轮时同样必须使用**。作用不是追踪多发言人（1v1 不需要），而是**保留对话的试探和推拉节奏**。

**判断标准**：如果一个 Q&A 后面紧跟的是对同一个话题的追问（而不是切换到新话题），且追问链 ≥3 轮，则从第 3 轮起使用 Re Q / Re A。

**典型错误**（Josh Cantor 案例，Timeline 讨论）：
```
❌ 错误：把 6 轮对话合并成 2 个 Q&A

✅ 正确：
Q：Compass 接下来 12 个月的 Timeline 是怎样的？
A：有 Voucher 他们可能在年底就能获批……
Re Q：也许他们就是在等 26 周数据来决定两剂能不能写进标签？
Re A：另外还有一点，也许他们希望赶在 11 月中期选举之前……
Re Q：我没想过这个
Re A：不排除这种可能……
Re Q：假设真的存在一点政治因素，审批速度比官方指引更快，你觉得可能吗？
Re A：也未必。
```

**关键原则**：追问链保留得越完整，纪要越能反映对话的真实动态。不要为了"整洁"而合并。
...
按参考纪要的格式，生成 docx 文件。**Q&A 严格按逐字稿时间顺序排列，禁止按主题归类重排。** 固定结构：
...
#### 英文保留与翻译规则（2026-07-09 Josh Cantor 教训）

纪要正文以中文为主。英文分三类处理：

```
☐ 第一类：保留英文 + 首次括号注中文（行业通用缩写，不翻译更清晰）
   - 监管/机构：FDA、DEA、CNMPA、EMA、PDUFA
   - 申报/注册：NDA、BLA、IND、REMS、sNDA
   - 临床术语：Ph2、Ph3、MADRS、HR、p 值
   - 商业/金融缩写：M&A、IP、CEO、TAM、CRO、VC
   - Ticker：CMPS、BBIO、NBIX 等
   - 药物/靶点：COMP360、acoramidis、5-HT2B 等
   - 试验编号：COMP006、CARDIO-TTRansform 等
☐ 第二类：必须翻译成中文（日常词/描述词/金融术语——保留英文就是偷懒）
   - 日常副词/连词："maybe""probably""basically""honestly""sort of"
   - 金融术语："consensus""launch""ramp""overhang""deal breaker""setup""call option"
   - 描述性词汇："cumbersome""nuanced""compelling""durability""agnostic""niche"
☐ 第三类：翻译成中文，但首次可在括号里附英文（帮助读者对应原文术语）
   - "功能性揭盲（functional unblinding）""市场无效定价（market inefficiency）"
   - 首次附英文，后文纯中文。不要每次都附
☐ 典型错误对照：
   ❌ "maybe 年底就能获批" → ✅ "最快可能年底获批"
   ❌ "consensus 数字暗示" → ✅ "市场一致预期（consensus）"
   ❌ "操作 cumbersome" → ✅ "操作繁琐"
   ❌ "launch ramp 慢于预期" → ✅ "上市放量慢于预期"
   ❌ "这是个 fundamentally well set up 的股票" → ✅ "这是个基本面非常好的股票"
   ❌ "免费 call options" → ✅ "免费看涨期权"
☐ 正确保留英文的示例：
   ✅ "FDA 可能以 5-HT2B 心脏瓣膜病为由要求补充数据"
   ✅ "TAM 500-750 亿美元"
   ✅ "DEA scheduling 进度、payer access 周期"
   ✅ "COMP360 获批后 DEA 须约 60 天内强制重新分类"
☐ 自查方法：全文搜索第二类检测词（maybe|probably|consensus|launch|ramp|overhang|cumbersome|nuanced|compelling|durability|agnostic|setup|deal breaker|niche|basically|honestly|call option），计数必须为 0。
```
...
☐ Q 文字不含括号注释（如"（团队、适应症拆分、入院进展）"）。问题文字本身应自明
☐ Q 文字不含两个问号。一个 Q 只问一件事。如果原文追问了多件事，拆成多个 Q
☐ Q 本身不使用破折号和箭头
...
```
☐ 先写 Q&A，再写 Key Takeaway。KT 是 Q&A 写完之后提炼出来的概括总结——从对话里捡出最重要的几条事实，不是另起炉灶写一份独立分析。
☐ ≤5 个主题组。超过 5 个说明你还没从 Q&A 里做完提炼。
...
☐ 每条 bullet 不超过 60 字（中文）。超过说明你在概括而非罗列事实
☐ 主题标签极简，不加英文全称、不加交易代码、不加"（前称XX）"之类注释。如"销售相关""CMG901"而非"CM310（Stapokibart）销售""CMG901（Claudin 18.2 ADC，AZD0901）"
☐ 4-5 个主题为宜，不超过 6 个。如果一个分子在本场只被顺带提到、没有展开讨论，不要单独开 topic
☐ 不要在 Key Takeaway 里做事实纠正（"注：讲话人误称..."）。纠正放在逐字稿和 Q&A 里
☐ 不使用箭头（→）和破折号（——）
...
❌ TAM 500-750亿美元。Josh 的核心框架：赛道需要足够大，大到每家领先企业拿5%份额就能支撑估值。不试图挑赢家，不同产品按疗效量级、体验时长、体验强度自然细分市场。
   ↑ 这是三段话揉成了一句。拆成三个 bullet：①TAM 规模；②5% 份额前提；③不挑赢家。
❌ COMP360 获批概率：Josh 认为 95%+。几乎没有安全性风险，临床获益真实。仅有 <1% 极端尾部风险：FDA 可能以 5-HT2B 心脏瓣膜病为由要求补充心脏毒性数据，但动物模型未提示风险、暴露时间极短。
   ↑ "仅有 <1%……极短"是一段微型论证，不是一句话一个事实。拆成两句：①获批概率 95%+；②极端尾部风险为 5-HT2B 心脏瓣膜病（<1% 概率）。
❌ 006 试验 26 周数据：两次给药缓解率显著优于 001 单次给药。Josh 认为数据差异化足够，两剂方案大概率写入标签。FDA 不会在等待额外 Ph3 期间限制患者获得最优治疗。
   ↑ 三句话。拆成三个 bullet。
```

**Key Takeaway 子弹的一事一议原则**：
- 如果一段话里包含"Josh 认为"+"数据支持"+"监管不会"，这是三个独立事实 → 拆成三个 bullet
- 如果一段话是"结论 + 原因 + 反证"，这是论证不是事实 → 只保留结论，原因放 Q&A
- 每条 bullet 自己读完就是一条完整的、不需要再看上下文就能理解的信息
...
☐ 自查方法：随机抽 3 段 A 文字，对照逐字稿原话。纪要 A 的字符数不应超过逐字稿对应段落的 50%。如果超过 50%，说明你在保留废话而非压缩。逐字稿 200 字的回答，纪要 A 应在 100 字以内
```
...
- 生成后验证：
  ...
  - 检查标题空格数（日期后只能有一个空格）
  - **英文混杂检查（2026-07-09 新增）**：全文搜索 maybe|probably|consensus|launch|ramp|overhang|cumbersome|nuanced|compelling|durability|agnostic|setup|deal breaker|niche|basically|honestly|call option，计数必须为 0（FDA/DEA/REMS/TAM/M&A 等行业缩写可保留）
- 生成脚本在验证通过后删除，不留垃圾文件
...
| **对叙述段落编造具体问题**（建投余老师第一版核心错误） | 没人问就不要编 Q。叙述块用"XX相关情况"做话题标签 |
| **1v1 开头独白直接跳过**（Josh Cantor 核心错误） | 独白不是话题标签也不是废话。重建 Q + 标注"录音未录到提问" |
| **Key Takeaway 子弹写成微型论证段落** | 一句话一个事实。每条检查：去掉"Josh 认为""数据表明""因此"后是否还是一个事实？ |
| **纪要英文混杂**（Josh Cantor 核心错误） | FDA/DEA/TAM/M&A 等行业缩写可保留。maybe/consensus/cumbersome/launch ramp 等日常词必须翻译 |
| **Key Takeaway 子弹中加英文括号注释** | "（rolling submission）""（众人拾柴火焰高）""（无需清淋预处理）"——删掉，翻译成中文或直接去掉注释 |
| **1v1 访谈连续追问合并为单一 Q&A**（Josh Cantor 核心错误） | ≥3 轮追问链必须用 Re Q / Re A 拆开，保留对话推拉节奏 |
| **多人对话合并成一个大 A** | 用 Re Q / Re A / 具名标签保留互动节奏 |
| **打乱逐字稿顺序，按自己逻辑重排** | 严格按逐字稿时间顺序。叙述在前就在前，追问在后就在后 |
| **1v1 访谈逻辑直接套到多人 presentation 会议** | 先判断会议类型（§E.2 Step 1.5 判断流程），再选处理模式 |
...
### E.8 自查清单（每次出纪要前过一遍，5 分钟）

两条根因，来自三次失败任务：

- **森松**：逐字稿说了什么就写什么，不要用自己的话概括。
- **建投医药余老师**：没人问就不要编 Q。叙述块用话题标签，真问答才用 Q&A。
- **Josh Cantor**：纪要不是翻译练习。中英文混杂、Key Takeaway 写微型论证、开头独白跳过、追问链合并成大 Q——本质是不把纪要当对话记录。

```
□ 概括检查（森松）：随机抽 5 个 A 段落，对照逐字稿原话。纪要的 A 文字是不是就是逐字稿去掉"我觉得""就是说"之后的原话？如果不是，你在概括。回去改成原话。
□ 编造检查（建投）：纪要中每个 Q 的文字，在逐字稿里能找到有人真的问出来吗？找不到 → 改成"XX相关情况"话题标签。
□ 顺序检查（建投）：纪要的 Q&A 顺序和逐字稿的对话顺序一致吗？不一致 → 你在重排，改回原顺序。
□ 粒度检查（森松）：随机抽 3 个话题区块，对照逐字稿数一下原始有几个问答来回。在纪要中，这几个来回是否都变成了独立 Q&A？（如果纪要约 20 个 Q 而逐字稿有 35+ 个问答交换，说明你合并了）
□ 多人对话检查（建投）：有 3+ 人参与吗？有 → 是否用了 Re Q / Re A / 具名标签追踪对话链？没用 → 补上。
...
□ 联网核实：股价、市值、催化剂状态是否已查？标日期了吗？
□ 英文混杂检查（Josh Cantor）：全文搜索 maybe|probably|consensus|launch|ramp|overhang|cumbersome|nuanced|compelling|durability|agnostic|setup|deal breaker|niche|basically|honestly|call option，计数必须为 0。（FDA/DEA/REMS/TAM/M&A 等行业缩写可保留）
□ 开头独白检查（Josh Cantor）：逐字稿开头是否 ≥3 段只有讲话人 1 的连续输出？有 → 是否已在纪要中用"（录音未录到提问，根据回答内容推测：……）"重建 Q 补入？
□ Key Takeaway 子弹长度检查（Josh Cantor）：随机抽 5 条 bullet，每条是否 ≤60 字（中文）？超过 → 你在写微型论证，拆开。
□ Re Q / Re A 检查（Josh Cantor）：1v1 访谈中有 ≥3 轮追问链吗？有 → 是否用了 Re Q / Re A 保留推拉节奏？没用 → 补上。
```
